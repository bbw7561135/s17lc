...
#
# History
#   This is a modified version of the routines Sumit Sabadhicary
#   used to generate radio lumosities in Sarbadhicary+17. This 
#   version avoides the use of numpy arrays in the luminosity
#   routine, and adds a routine to generate radio light cuvers
#   for a SN given various parameters
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
def luminosity(rs, vs, n0, epse, p, nu):
    ...
    #OLD comment all the numpy arrays
    # rs=np.array(rs)
    # vs=np.array(vs)
    # n0=np.array(n0)
    # epse=np.array(epse)
    ...
    #OLD epsCR=np.array(epsCR)
    #OLD epsCR=np.select([epsCR>0.1],[0.1],default=epsCR)
    ...
    #OLD B=np.array(B)
    #OLD B=np.select([B<4*B0],[4*B0],default=B)
...
def do_one(n0=1, mej=3, e51=1, epse=0.04,p=2.1,nu=1.4,sntype='cc'):
    '''
    Calculate the radio luminosity of a SNR as a function of time
    '''
    ...
    v=[]
    for one_time in t:
        one_r,one_v=radius_velocity(one_time, n0, mej, e51, sntype)
        r.append(one_r)
        v.append(one_v)

    x=Table([t,r,v],names=['Age','R','v'])

    lum=[]
    for one in x:
        one_lum=luminosity(one['R'], one['v'], n0, epse, p, nu)
        lum.append(one_lum)
